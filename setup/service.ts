...
<dict>
  ...
  try {
    ...
    serviceLoaded = output.includes('com.flexclaw');
  } catch {
...
/**
 * Kill any orphaned flexclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned flexclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/flexclaw.service';
    ...
    unitPath = path.join(unitDir, 'flexclaw.service');
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned flexclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable flexclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start flexclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active flexclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-flexclaw.sh');
  const pidFile = path.join(projectRoot, 'flexclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-flexclaw.sh — Start FlexClaw without systemd',
    ...
    '    echo "Stopping existing FlexClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting FlexClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/flexclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/flexclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "FlexClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/flexclaw.log"`,
  ];
